...
`UAV_INFO` 只提供“飞向哪里”，`DYT_GUIDANCE_COMMAND phase=2` 才表示“现在开始或恢复中制导”。仅发送目标位置不能替代开启指令；仅发送开启指令但没有有效目标位置，飞机进入 Offboard 后只能保持位置，不能正常跟随目标。
...
## 6. 状态反馈：`DYT_SYSTEM_STATUS(12926)`
...
- 如果需要在任何 RTL 下明确继续中制导，不依赖自动恢复结果，按第 5 节发送新的 `DYT_GUIDANCE_COMMAND phase=2`。
